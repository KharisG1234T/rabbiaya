...
class Pengajuan extends CI_Controller
{
  ...
  private function _persingData($data, $isArchieve = false)
  {
    ...
    foreach ($data as $idx => $item) {
      ...
      switch ($itemArray['status']) {
        case "PROCESS":
          $userApprovalData = $this->Pengajuan_model->getUserApprovalByPengajuanId($itemArray['id']);
      ...
      if ($isArchieve) {
        ...
        if ($this->session->userdata("role_id") == 1) {
          $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/restore/') . $itemArray['id'] . '" onclick="return confirm(' . "'Anda yakin ingin me-restore data ini?'" . ')"><i class="fas fa fa-recycle"></i>&nbsp;&nbsp; Restore</a>';
        }
      } else {
        // btn action for list actived data

        // Menggabungkan nilai dari kolom userApprovals dan keterangan_sku
        $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/detail/') . $itemArray['id'] . '"><i class="fas fa fa-eye"></i>&nbsp;&nbsp; Detail</a>';
        ...
        if ($itemArray["status"] == "PENDING") {
          if (in_array($roleId, array(1, 2))) {
            $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/edit/') . $itemArray['id'] . '"><i class="fas fa fa-pen"></i>&nbsp;&nbsp; Perbarui</a>';
          }
        }
        ...
        if ($itemArray["status"] == "PROCESS" && in_array($this->session->userdata('role_id'), array(4, 5, 6, 7, 8))) {
          $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/approve/') . $itemArray['id'] . '"><i class="fas fa fa-check-double"></i>&nbsp;&nbsp; Approve</a>';
        }

        // Bukan Sales dan PM
        if ($itemArray["status"] == "PROCESS" && in_array($this->session->userdata('role_id'), array(1, 3, 8, 9, 10))) {
          $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/reject/') . $itemArray['id'] . '"><i class="fas fa fa-minus"></i>&nbsp;&nbsp; Tolak</a>';
        }

        // Admin
        if ($itemArray["status"] == "REJECTED" && in_array($this->session->userdata('role_id'), array(1))) {
          $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/unreject/') . $itemArray['id'] . '"><i class="fas fa fa-recycle"></i>&nbsp;&nbsp; Batal Tolak</a>';
        }

        // Admin atau Sales
        if ($this->session->userdata('role_id') == 1) {
          $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/delete/') . $itemArray['id'] . '" onclick="return confirm(' . "'Anda yakin ingin menghapus data ini?'" . ')"><i class="fas fa fa-trash"></i>&nbsp;&nbsp; Hapus</a>';
        }

        // Admin atau Sales
        if (in_array($this->session->userdata('role_id'), array(1, 2, 4))) {
          $action .= '<a class="dropdown-item" href="' . base_url('pengajuan/print/') . $itemArray['id'] . '"><i class="fas fa fa-print"></i>&nbsp;&nbsp; Cetak</a>';
        }
      }
  ...
  public function datatable()
  {
    ...
    $from = $this->input->get('area_id')  ?? "ALL";
    $isArchieve = false;
    $order_by = $this->input->get('data') ?? 'kode_pengajuan';
    $order_direction = $this->input->get("order[0][dir]") ?? 'desc';
  ...
  public function insert()
  {
    ...
    // Cek area_id dari tabel user_area berdasarkan user_id
    $query = $this->db->select('area_id')->from('user_area')->where('user_id', $userId)->get();
    $userArea = $query->row();
    $areaId = $userArea ? $userArea->area_id : NULL;
    ...
    // Setelah pengajuan disimpan, ambil ID pengajuan yang baru
    ...
    // Mengambil data destinasi perjalanan dari form
    ...
    // Mengambil data detail perjalanan dari form
    $official_trip_detail = $this->input->post('official_trip_detail');
    foreach ($official_trip_detail as $trip_detail) {
      $data = array(
        'official_trip_id' => $PengajuanId,
        'official_trip_activity_id' => $trip_detail['official_trip_activity_id'],
  ...
  public function update()
  {
    ...
    try {
      ...
      // Hapus data lama
      $this->Official_trip_destination_model->deleteByOfficialTripId($idPengajuan);
      $this->Official_trip_detail_model->deleteByOfficialTripId($idPengajuan);

      // Update data pengajuan (tanpa memperbarui kode_pengajuan)
      $this->Pengajuan_model->update($dataPengajuan, $idPengajuan);

      // Simpan data yang diedit untuk official trip destination
      $official_trip_destination = $this->input->post('official_trip_destination');
      foreach ($official_trip_destination as $trip_destination) {
        $data = array(
          'official_trip_id' => $idPengajuan,
          'name' => $trip_destination['name'],
          'destination' => $trip_destination['destination'],
          'remark' => $trip_destination['remark'],
          'ticket_number' => $trip_destination['ticket_number'],
        ...
        log_message('debug', 'Data Trip Destination: ' . json_encode($data));

        $this->Official_trip_destination_model->save($data);
      }

      // Simpan data yang diedit untuk official trip detail
      $official_trip_detail = $this->input->post('official_trip_detail');
      foreach ($official_trip_detail as $trip_detail) {
        $data = array(
          'official_trip_id' => $idPengajuan,
        ...
        log_message('debug', 'Data Trip Detail: ' . json_encode($data));
      ...
      // Cek status transaksi, jika gagal, lemparkan exception
  ...
  public function edit($id)
  {
    ...
    // $data['official_trip_details'] = $this->Official_trip_detail_model->getAllBy($id);
    // $data['official_trip_destinations'] = $this->Official_trip_destination_model->getAllBy($id);
    $data['activity_options'] = $this->Official_trip_activity_model->getAll(); // Misalnya ini adalah model untuk mengambil data aktivitas
  ...
  public function detail($id_pengajuan)
  {
    ...
    // Default values for approval statuses
    $approvalRoles = ['sales', 'ks', 'hr', 'hrd'];
    foreach ($approvalRoles as $role) {
      $data['pengajuan']['approve'][$role] = ['ttd' => 'waiting.png', 'createdat' => ''];
    }

    if (isset($data['pengajuan']['official_trip_approval']['users'])) {
      foreach ($data['pengajuan']['official_trip_approval']['users'] as $user) {
        switch ($user['role_id']) {
          case 2:
      ...
      // Unset array official_trip_approval
      unset($data['pengajuan']['official_trip_approval']);
    }
